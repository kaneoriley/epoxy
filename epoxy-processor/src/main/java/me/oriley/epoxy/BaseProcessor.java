...
abstract class BaseProcessor extends AbstractProcessor {
    ...
    @NonNull
    public String getPackageName(@NonNull Element type) {
        return mElements.getPackageOf(type).getQualifiedName().toString();
    }

    @NonNull
    public Element asElement(@NonNull TypeMirror type) {
        return mTypes.asElement(type);
    }

    @NonNull
    public String getClassName(@NonNull Element type) {
        if (!(type instanceof TypeElement)) {
            throw new IllegalArgumentException("Element " + type.toString() + " is not a valid TypeElement");
        }
    ...
    public boolean isAssignable(@NonNull TypeMirror typeMirror, @NonNull Class c) throws EpoxyException {
        TypeElement typeElement = mElements.getTypeElement(c.getCanonicalName());
        if (typeElement == null) {
            throw new EpoxyException("Element %s could not be retrieved. Are you sure it's not primitive?");
        }
    ...
    @Nullable
    protected static Element findEnclosingElement(@NonNull Element e) {
    ...
    public void throwError(@NonNull String message, @NonNull Exception e, @Nullable Object... args) {
        error(message, args);
